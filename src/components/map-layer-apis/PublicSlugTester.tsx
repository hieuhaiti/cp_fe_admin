...
export default function PublicSlugTester() {
  ...
  const registriesQuery = useApiQuery(
    ['mapLayerApis', 'slug-picker'],
  ...
  const registries = useMemo(() => {
    const data = (registriesQuery.data as any)?.data
    const items = data?.items ?? data?.apis ?? []
    return Array.isArray(items) ? items.filter((r: any) => r.slug) : []
  ...
  async function handleTest(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!slug.trim() || !apiKey.trim()) {
      setError('Vui lòng nhập slug và token chia sẻ')
      return
    }
        ...
        <CardContent className="space-y-4">
          <form className="space-y-4" onSubmit={handleTest}>
            <div className="space-y-2">
              <Label htmlFor="public-slug" className="inline-flex items-center gap-2">
              ...
              <div className="flex gap-1.5">
                <Input
                  ...
                  placeholder="thuy-loi-cam-pha"
                  className="flex-1"
                />
                {registries.length > 0 && (
                  <Select value="" onValueChange={setSlug}>
                    <SelectTrigger
                      className="w-auto shrink-0 px-3"
                      aria-label="Chọn slug từ danh sách"
                    >
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="eyJ... (token trả về khi cấp / xoay key)"
              />
              <p className="text-muted-foreground text-xs">
                Dùng token JWT được cấp một lần khi tạo hoặc xoay key — không phải API key admin.
              </p>
            </div>
