...
function RunFormFields({
  ...
  if (module === 'rain') {
    const source = value('source') || 'IMERG'
      ...
      <div className="space-y-5">
        <div className="flex items-start gap-3 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-950">
          ...
          <p>
            ...
            Kết quả cần được đối chiếu với quan trắc và thông tin hiện trường trước khi ra quyết
            định.
          </p>
        </div>
        ...
        <div className="space-y-3">
          <div>
            <Label>Nguồn dữ liệu mưa</Label>
            <p className="text-muted-foreground mt-1 text-xs">
              Chọn dữ liệu vệ tinh tự động hoặc tự nhập số liệu đo tích lũy.
            </p>
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            <button
              type="button"
              aria-pressed={source === 'IMERG'}
              onClick={() => onChange('source', 'IMERG')}
              className={cn(
                'focus-visible:ring-ring relative flex min-h-28 items-start gap-3 rounded-lg border p-4 text-left transition-colors focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none',
                source === 'IMERG'
            ...
            >
              ...
              <span className="min-w-0">
                ...
                <span className="text-muted-foreground mt-1 block text-xs leading-5">
                  Hệ thống truy xuất dữ liệu mưa vệ tinh theo ngày sự kiện.
                </span>
              </span>
              {source === 'IMERG' ? (
                <CheckCircle2 className="absolute top-3 right-3 size-4 text-sky-700" />
              ) : null}
            </button>
            ...
            >
              ...
              <span className="min-w-0">
                <span className="font-medium">Nhập số liệu đo</span>
                <span className="text-muted-foreground mt-1 block text-xs leading-5">
                  Dùng tổng lượng mưa từ trạm đo hoặc nguồn nghiệp vụ đã kiểm tra.
                </span>
              </span>
              {source === 'MANUAL' ? (
                <CheckCircle2 className="absolute top-3 right-3 size-4 text-sky-700" />
        ...
        <div className="flex items-start gap-3 rounded-lg border bg-slate-50 p-3 text-sm">
          <Lock className="mt-0.5 size-4 shrink-0 text-slate-600" />
          <div>
            <p className="font-medium">
              Ngưỡng cảnh báo {value('threshold') || '0,6'} được server quản lý
            </p>
            <p className="text-muted-foreground mt-1 text-xs">
              Áp dụng theo cấu hình {configVersion || 'đang tải'}. Người vận hành không cần nhập
              hoặc thay đổi tham số khoa học này.
            </p>
          </div>
        </div>
        ...
        {source === 'IMERG' ? (
          <div className="rounded-lg border p-4">
            <div className="mb-4 flex items-start gap-3">
              ...
              <div>
                <p className="font-medium">Chọn ngày sự kiện mưa</p>
                <p className="text-muted-foreground mt-1 text-xs">
                  IMERG sẽ cung cấp lớp mưa không gian; hệ thống tự kết hợp dữ liệu mưa tiền kỳ cần
                  thiết cho mô hình.
                </p>
              </div>
            </div>
            <div className="max-w-sm">
              {dateField('eventTime', 'Ngày xảy ra mưa *', 'Dùng ngày theo giờ UTC của hệ thống.')}
            </div>
          </div>
        ) : (
          <div className="space-y-4 rounded-lg border p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="font-medium">Nhập tổng lượng mưa tích lũy</p>
                <p className="text-muted-foreground mt-1 text-xs">
                  Đơn vị mm. Nhập ít nhất một khoảng thời gian từ số liệu đã được kiểm tra.
                </p>
              </div>
            </div>
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
              {rainfallFields.map(([key, label]) => (
                <div key={key}>{numberField(key, `Trong ${label}`)}</div>
              ))}
            </div>
            <OpenWeatherReference />
          </div>
        )}
      </div>
    )
