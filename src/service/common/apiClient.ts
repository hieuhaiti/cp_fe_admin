...
function buildQuery(url: string, opts: RequestOptions = {}): string {
  ...
  // The Postman collection is the API contract. None of its request URLs
  // accepts a global `lang` query parameter, so only forward parameters that
  // the caller explicitly provides for that endpoint.
  delete params.lang
...
async function refreshAccessToken(initialRefreshToken: string): Promise<string | null> {
  ...
  return lockManager.request('campha-admin-auth-refresh', () =>
...
export async function post<T = any>(
  ...
  const headers = buildHeaders(opts, hasBody && !useForm)
  const body = useForm ? data : hasBody ? JSON.stringify(data) : undefined
