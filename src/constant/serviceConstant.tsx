// Auth
export const serviceAuthPath = '/auth'

// Admin Users (Postman: /admin/users)
export const serviceAdminUserPath = '/admin/users'
// Legacy alias (kept for pages not migrated yet)
export const serviceUserPath = '/admin/users'

// Notifications
export const serviceNotificationPath = '/notifications'
export const serviceNotificationDevicesPath = '/notifications/devices'

// News
export const serviceNewsPath = '/cms/news'
export const serviceAdminNewsPath = '/admin/cms/news'
export const serviceAdminNewsCommentsPath = '/admin/cms/news/comments'
export const serviceAdminCommentsPath = '/admin/comments'

// Feedback (Postman)
export const serviceFeedbackPath = '/field-reports'
export const serviceAdminFeedbackPath = '/admin/field-reports'

// PDF Maps (replaces map-images)
export const servicePdfMapPath = '/cms/pdf-maps'
export const serviceAdminPdfMapPath = '/admin/cms/pdf-maps'
// Legacy alias
export const serviceMapImagePath = '/cms/pdf-maps'

// Map / GeoServer
export const serviceMapLayerPath = '/admin/layers'
export const serviceMapImportJobPath = '/admin/layers/imports'
export const serviceMapRasterHarvestPath = '/map/rasters'

// Map APIs (third-party sharing)
export const serviceMapApiPath = '/admin/api-registry'
export const serviceMapDataPath = '/shared'
// Legacy aliases
export const serviceMapLayerApiPath = '/map-apis'

// Weather
export const serviceWeatherPath = '/weather'

// Remote Sensing
export const serviceRemoteSensingPath = '/remote-sensing'
export const serviceAdminRemoteSensingPath = '/admin/remote-sensing'

// Statistics
export const serviceStatsPath = '/statistics'
// Legacy alias
export const serviceStatisticsPath = '/statistics'

// Flood / Hydrology
export const serviceAdminFloodPath = '/admin/flood'
export const serviceFloodPath = '/field-reports'
export const serviceAdminFieldReportPath = '/admin/field-reports'

// Satellite (on-demand GEE)
export const serviceSatellitePath = '/satellite'

// Forest Classification
export const serviceForestClassificationPath = '/forest-classification'

// Field measurements
export const serviceFieldMeasurementPath = '/field-measurements'
export const serviceMonitoredAreaPath = '/monitored-areas'

// Legacy modules (kept as-is, not in Postman)
export const serviceNewsCommentPath = '/news-comments'
export const serviceAuditLogPath = '/audit-logs'
